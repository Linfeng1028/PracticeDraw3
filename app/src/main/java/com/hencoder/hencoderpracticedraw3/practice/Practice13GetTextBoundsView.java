...
public class Practice13GetTextBoundsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(text1, 100, middle + height, paint2);

        paint2.getTextBounds(text2, 0, 1, bounds);
        height = -(bounds.bottom + bounds.top) / 2;
        canvas.drawText(text2, 200, middle +height, paint2);

        paint2.getTextBounds(text3, 0, 1, bounds);
        height = -(bounds.bottom + bounds.top) / 2;
        canvas.drawText(text3, 300, middle +height, paint2);

        paint2.getTextBounds(text4, 0, 1, bounds);
        height = -(bounds.bottom + bounds.top) / 2;
        canvas.drawText(text4, 400, middle +height, paint2);

        paint2.getTextBounds(text5, 0, 1, bounds);
        height = -(bounds.bottom + bounds.top) / 2;
        canvas.drawText(text5, 500, middle +height, paint2);

        paint2.getTextBounds(text6, 0, 1, bounds);
        height = -(bounds.bottom + bounds.top) / 2;
        canvas.drawText(text6, 600, middle +height, paint2);
    }
}